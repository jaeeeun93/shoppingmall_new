...
<th colspan=6 style="font-weight:bold; background-color:lightgray;">배송물품</th>
	...
	<tr>
		<td><input id="cartFile_s" type="image" name="cartFile_s" src="/upload/${cartFile_s }"></td>
		<td><input id="itemName" name="itemName" value="${itemName }"></td>
		<td><input id="itemPrice" name="itemPrice" value="${price}원"></td>
		<input type="hidden" id="orderId" name="orderId" value="${orderId}">
	</tr>
	
<th colspan=6 style="font-weight:bold; background-color:lightgray;">적립포인트</th>
	...
	<tr>
		...
		<tr>
			<td>결제방법</td>
			<td colspan=2><input type="radio" name="pay" value="카카오페이" checked>카카오 페이</td>
		</tr>
	</tr>
	<tr>
	...
	<tr>
		<td colspan=3 align=center style="background-color:lightgray;"><button type="button" onclick="location.href='/order/OrderCancel.do?orderId=${orderId}'">쇼핑 목록으로 돌아가기</button></td>
	</tr>
</table>
</form>
<!-- 여기부터 카카오페이 결제 구현 -->
            ...
            */
            ...
            if (rsp.success) {
                ...
                msg += '카드 승인번호 : ' + rsp.apply_num;
                 ...
                 location.href="/order/OrderBuy.do?id=${customer.id}&name=${customer.name}&email=${customer.email}&phone=${customer.phone}&cname="+cname+"&zipcode="+zipcode+"&zip1="+zip1+"&zip2="+zip2+"&zip3="+zip3+"&cphone="+cphone+"&totalPrice="+total+"&orderId=${orderId}&pick="+pick+"&coupon="+coupon+"&itemPoint=${itemPoint}&itemCode=<%=itemCode%>";
