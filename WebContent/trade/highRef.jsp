...
<c:if test="${count  > 0}">
	...
	<c:if test="${startPage > pageBlock }">
		<a href=/trade/hignRef.do?pageNum=${startPage - pageBlock }">[이전]</a>
	</c:if>
	
	<!-- 페이지 출력 -->
	<c:forEach var="i" begin="${startPage }" end="${endPage }">
		<a href="/trade/hignRef.do?pageNum=${i }">[${i }]</a>
	</c:forEach>
	
	<!-- 다음 링크 -->
	<c:if test="${endPage < pageCount}">
		<a href="/trade/hignRef.do?pageNum=${startPage + pageBlock}">[다음]</a>
	</c:if>
</c:if>
		</td>
	</tr>
...
<table width=90% align=center>
	<tr>
		<td align=left><a href="/trade/hignRef.do">새로고침</a></td>
		<td align=right>
...
<div class="search">
<form action="/trade/hignRef.do" method="post">
<table align=center>
  <tr>
    <td>
   		<select name="field">
