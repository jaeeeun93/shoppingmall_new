		...
		<td width=30% height=100% valign=top>
			<table cellspacing=0 border=1 width=100%>
				...
				<c:choose>
					<c:when test="${trade.deal eq '3'}">
						<td align=center>
							<strike><fmt:formatNumber value="${trade.price}" type="currency"/>
						</td>
					</c:when>
					<c:otherwise>
						<td align=center>
...
<table width=90% align=center>
	<tr>
			<td align=left><a href="/trade/List.do">새로고침</a></td>
...
<c:if test="${count  > 0}">
	...
	<c:if test="${startPage > pageBlock }">
		<a href="/trade/List.do?pageNum=${startPage - pageBlock }">[이전]</a>
	</c:if>
	
	<!-- 페이지 출력 -->
	<c:forEach var="i" begin="${startPage }" end="${endPage }">
		<a href="/trade/List.do?pageNum=${i }">[${i }]</a>
	</c:forEach>
	
	<!-- 다음 링크 -->
	<c:if test="${endPage < pageCount}">
		<a href="/trade/List.do?pageNum=${startPage + pageBlock}">[다음]</a>
	</c:if>
</c:if>
<td align=right>
...
<div class="search">
<form action="/trade/List.do" method="post">
<table align=center>
  <tr>
    <td>
   		<select name="field">
